...
const peachShineDeals = [
  {image:'/assets/catalog/f6f20dd9cb8728f7.webp', title:'[농할 25%쿠폰] 연간 50만 송이 판매, 국내산 고당도 수출용 상주 샤인머스켓 (17brix)', original:'17,400원', sale:'20%', price:'13,900원', review:'7013+'},
  {image:'/assets/catalog/254111cacade414e.webp', title:'[농할 25%쿠폰] 당도선별, 국내산 우수산지 딱딱이 복숭아 1.5kg / 3kg (중과/대과)', original:'22,400원', sale:'20%', price:'17,900원', review:'5124+'},
];
const cakeItems = [
...
const preserveItems = [
  {image:'/assets/best/9b3a62f49667dc508061cab7112266a4.webp', title:'[새벽에On] 원물 그대로 킵! 백도 황도 병조림 450g (2+1 행사)', original:'20,000원', sale:'26%', price:'14,900원', review:'757'},
  {image:'/assets/catalog/826a7a95b30d88cd.webp', title:'생 복숭아 식감을 살린, 병조림 선물세트(백도&황도)', original:'', sale:'', price:'26,800원', review:'132'},
];
...
function DealCards({ items, go }: { items: {image:string;title:string;original:string;sale:string;price:string;review:string}[]; go?: (to:string)=>void }) {
  return <div className="deal-list">{items.map(item=><article className="deal-card" key={item.title}><button className="deal-img" onClick={()=>go?.(productPath(item.title))}><img src={item.image} alt=""/><i>장바구니</i></button><div className="deal-info"><p onClick={()=>go?.(productPath(item.title))}>{item.title}</p><b>{item.sale?<strong>{item.sale}</strong>:null}<span>{item.price}</span>{item.original?<del>{item.original}</del>:null}</b><q>{item.review}</q></div></article>)}</div>;
}
function HomeDeal({ go, clock }: { go:(to:string)=>void; clock:string }) {
  const [tab,setTab]=useState(0);
  const chips=['🍑납-작복숭아&햇사과 오픈 특가🍎','복숭아&샤인 특가'];
  return <section className="deal"><div className="chips">{chips.map((x,i)=><button className={i===tab?'selected':''} key={x} onClick={()=>setTab(i)}>{x}</button>)}</div><p className="deal-time"><span>{clock}</span></p><DealCards items={tab===0?dealItems:peachShineDeals} go={go}/><button className="deal-all" onClick={()=>go('products')}><span>특가 전체보기</span></button></section>;
}
function CakeDeal({ go }: { go:(to:string)=>void }) {
  const [clock,setClock]=useState(cakeRemain);
  const [tab,setTab]=useState(0);
  useEffect(()=>{ const id=setInterval(()=>setClock(cakeRemain()),1000); return()=>clearInterval(id); },[]);
  const chips=['생과일 케이크 오픈🍰','병조림 2+1 이벤트'];
  return <section className="deal deal-cake"><div className="chips">{chips.map((x,i)=><button className={i===tab?'selected':''} key={x} onClick={()=>setTab(i)}>{x}</button>)}</div><p className="deal-time"><span>{clock}</span></p><DealCards items={tab===0?cakeItems:preserveItems} go={go}/><button className="deal-all" onClick={()=>go('products')}><span>특가 전체보기</span></button></section>;
}
function RailSwiper({ className, children, centered }: { className: string; children: React.ReactNode; centered?: boolean }) {
  return centered
    ? <Swiper className={className} slidesPerView="auto" centeredSlides spaceBetween={10} loop grabCursor>{children}</Swiper>
    : <Swiper className={className} modules={[FreeMode]} freeMode slidesPerView="auto" spaceBetween={10} grabCursor>{children}</Swiper>;
}
...
function ShowcaseRail({ className, title, subtitle, items, ranked, go, swipe = true }: { className: string; title: string; subtitle: string; items: CatalogItem[]; ranked?: boolean; go: (to:string)=>void; swipe?: boolean }) {
  const cards = items.map((item,i)=><CatalogCard item={item} rank={ranked?i+1:undefined} go={go} key={`${item.id}-${i}`}/>);
  return <section className={`source-module ${className}`}><div className="module-head"><div><h2>{title}</h2><p>{subtitle}</p></div>{ranked?null:<button onClick={()=>go('products')}>더보기</button>}</div>{swipe?<RailSwiper className={`source-rail${ranked?' rank-rail':''}`} centered={ranked}>{items.map((item,i)=><SwiperSlide key={`${item.id}-${i}`}><CatalogCard item={item} rank={ranked?i+1:undefined} go={go}/></SwiperSlide>)}</RailSwiper>:<div className={`source-rail${ranked?' rank-rail':''}`}>{cards}</div>}{ranked?<button className="rank-more" onClick={()=>go('products')}><span>실시간 랭킹 과일 전체보기</span></button>:null}</section>;
}
...
function SourceSalePage({ go }: { go:(to:string)=>void }) {
  ...
  return <main className="source-sale-page">{top.map(src=><img src={src} alt="" key={src}/>)}<section className="newpage-coupon"><img className="newpage-coupon__shot" src="/assets/sale/coupon.png" alt="8월 농식품부 할인지원 25%"/></section><section className="newpage-product"><div className="sale-rail"><RailSwiper className="sale-swiper">{rail.map(item=><SwiperSlide key={item.rank}><button className="sale-rail-card" onClick={()=>go(productPath(item.title))}><span className="sale-thumb"><img src={item.image} alt=""/><i>장바구니</i></span><p>{item.title}</p><del>{item.original}원</del><b><strong>{item.sale}</strong><span>{item.price}</span></b>{item.benefit&&<u>최대 혜택가 <em>{item.benefit}</em></u>}</button></SwiperSlide>)}</RailSwiper></div><button className="newpage-pd__more" type="button" onClick={()=>go('dawn')}><span>전체보기</span></button></section>{bottom.map(src=><img src={src} alt="" key={src}/>)}<footer className="sugar-footer"><div className="bottom__area"><button type="button" className="bottom--toggle-btn">㈜온브릭스 사업자 정보</button></div><ul className="bottom__menu"><li><button type="button" onClick={()=>go('service')}>고객센터</button></li><li><button type="button">대량주문문의</button></li><li><button type="button" onClick={()=>go('legal')}>이용약관</button></li><li><button type="button" onClick={()=>go('legal')}>개인정보처리방침</button></li></ul></footer></main>;
}
...
function SourceProductList({ go }: { go:(to:string)=>void }) {
  ...
  const filtered=cat===0?bestList:cat===1?bestList.filter(i=>/국내산|햇사레|온리브릭스/.test(i.title)&&!/캐나다|제스프리|태국|항공/.test(i.title)):cat===2?bestList.filter(i=>/캐나다|제스프리|태국|항공|수입/.test(i.title)):bestList.filter(i=>/선물|세트|바구니/.test(i.title));
  return <main className="source-list-page"><div className="source-category">{listCategories.map(([image,label],i)=><button className={i===cat?'on':''} key={label} onClick={()=>setCat(i)}><img src={image} alt=""/><span>{label}</span></button>)}</div><div className="source-list-top"><b>총 {filtered.length}개</b><button>랭킹순</button></div><div className="source-product-grid">{filtered.map((item,i)=><button className="source-list-card" onClick={()=>go(productPath(item.title))} key={item.rank}><i className="list-rank">{i+1}</i><span className="list-image"><img src={item.image} alt=""/><em>장바구니</em></span><p>{item.title}</p>{item.original&&<del>{item.original}</del>}<b>{item.sale&&<strong>{item.sale}</strong>}<span>{item.price}</span></b>{item.benefit&&<u>최대 혜택가 <span>{item.benefit}</span></u>}{item.detail&&<s>자세히보기</s>}{item.badge&&<mark>{item.badge}</mark>}{item.review&&<q>{item.review}</q>}</button>)}</div><footer className="home-footer list-footer"><button type="button" className="bottom--toggle-btn">㈜온브릭스 사업자 정보</button><ul className="bottom__menu"><li><button onClick={()=>go('service')}>고객센터</button></li><li><button>대량주문문의</button></li><li><button onClick={()=>go('legal')}>이용약관</button></li><li><button onClick={()=>go('legal')}>개인정보처리방침</button></li></ul></footer></main>;
}
  ...
  return <main className={`source-detail ${product.id}-page`}>
    ...
    <section className="prd-info">
      ...
      <article className="review-block">
        ...
        <div className="review-rail"><RailSwiper className="review-swiper">{(detailReviews[product.id]?.items??detailReviews.melon.items).map(r=><SwiperSlide key={r.date+r.text.slice(0,8)}><article className="review-card"><div className="img"><img src={r.image} alt=""/></div><div className="info"><div className="info-top"><span className="star"/><b>5</b><p className="data">{r.date}</p></div><div className="text">{r.text}</div></div></article></SwiperSlide>)}</RailSwiper></div>
      </article>
...
function SourceSugarPage({ go }: { go:(to:string)=>void }) {
  ...
  return <main className="source-sugar">
    <div className="thumbs-area">
      <div className="thumbs-area__top"><button className="date-btn date-btn--prev" aria-label="이전" onClick={()=>mainRef.current?.slidePrev()} disabled={active===0}/><button className="date"><span>2026년 8월 20일</span></button><button className="date-btn date-btn--next" aria-label="다음" onClick={()=>mainRef.current?.slideNext()} disabled={active===sugarDiary.length-1}/></div>
    </div>
    <Swiper className="sugar-list" onSwiper={s=>{mainRef.current=s;}} onSlideChange={s=>setActive(s.activeIndex)} initialSlide={0} grabCursor>
...
function SourceStoryPage({ go }: { go:(to:string)=>void }) {
  const cats=['전체','월간 과일','후르츠 로드','온스토리','과일생활'];
  const extra=[{id:100,image:'/assets/detail/story/story-apple-2.webp',category:'후르츠 로드',title:'일상의 당도를 높이는 과일',body:'달콤한 과일 루틴 실천하기'},{id:101,image:'/assets/detail/story/story-blueberry-1.webp',category:'후르츠 로드',title:'동계 후르츠 리그',body:'나의 겨울 스포츠 타입은?'}];
  const [cat,setCat]=useState(0);
  const [page,setPage]=useState(1);
  const all=[...storyList,...extra];
  const filtered=cat===0?all:all.filter(s=>s.category===cats[cat]);
  const per=5;
  const pages=Math.max(1,Math.ceil(filtered.length/per));
  const shown=filtered.slice((page-1)*per,page*per);
  const nums=Array.from({length:Math.min(5,pages)},(_,i)=>i+1);
  return <main className="source-story-page"><div className="story-top"><div className="story-categories"><button className="story-bookmark" aria-label="북마크"/>{cats.map((x,i)=><button className={i===cat?'on':''} key={x} onClick={()=>{setCat(i);setPage(1);}}>{x}</button>)}</div><b className="story-scope">모든과일</b></div><div className="story-sort"><b>총 <em>{filtered.length} 개</em></b><button>최근순</button></div><section>{shown.map(item=><article key={item.id}><img src={item.image} alt=""/><div className="story-info"><div className="story-info-top"><span>{item.category}</span><button aria-label="북마크"/></div><h2>{item.title}</h2><p>{item.body}</p></div></article>)}</section><div className="story-paging"><button className="arrow" onClick={()=>setPage(1)}>{'<<'}</button><button className="arrow" onClick={()=>setPage(p=>Math.max(1,p-1))}>{'<'}</button><span className="page-group">{nums.map(n=><button className={n===page?'active':''} key={n} onClick={()=>setPage(n)}>{n}</button>)}</span><button className="arrow" onClick={()=>setPage(p=>Math.min(pages,p+1))}>{'>'}</button><button className="arrow" onClick={()=>setPage(pages)}>{'>>'}</button></div><footer className="home-footer story-footer"><button type="button" className="bottom--toggle-btn">㈜온브릭스 사업자 정보</button><ul className="bottom__menu"><li><button onClick={()=>go('service')}>고객센터</button></li><li><button>대량주문문의</button></li><li><button onClick={()=>go('legal')}>이용약관</button></li><li><button onClick={()=>go('legal')}>개인정보처리방침</button></li></ul></footer></main>;
}
...
function HomeStories({ go, stories }: { go:(to:string)=>void; stories:[string,string,string,string][] }) {
  const cats=['월간 과일','후르츠 로드','온스토리','과일생활'];
  const extra:[string,string,string,string][]=[['/assets/detail/story/story-apple-2.webp','후르츠 로드','일상의 당도를 높이는 과일','달콤한 과일 루틴 실천하기'],['/assets/detail/story/story-blueberry-1.webp','후르츠 로드','동계 후르츠 리그','나의 겨울 스포츠 타입은?'],['/assets/detail/story/story-blueberry-2.webp','온스토리','과일이 좋아하는 콜드체인 보관&배송','겨울은 얼지 않게, 여름은 무르지 않게'],['/assets/detail/story/story-grapes-1.webp','과일생활','포도 세척·보관법','포도 씻을 때 알맹이 떼면 안 되는 이유'],['/assets/detail/story/story-apple-1.webp','과일생활','사과 세척·보관법','냉장고에 넣었는데 푸석해졌다면?']];
  const [tab,setTab]=useState(0);
  const shown=[...stories,...extra].filter(s=>s[1]===cats[tab]);
  return <section className="source-story"><div className="module-head"><h2>온브릭스 스토리</h2><button className="story-more" onClick={()=>go('story')}><span>더보기</span></button></div><div className="story-chips">{cats.map((x,i)=><button className={i===tab?'selected':''} key={x} onClick={()=>setTab(i)}>{x}</button>)}</div><RailSwiper className="story-rail">{shown.map(([image,cat,line,title])=><SwiperSlide key={title}><button onClick={()=>go('story')}><img src={image} alt=""/><div className="story-card-info"><div className="story-cat"><span>{cat}</span><p>{line}</p></div><p className="story-card-title">{title}</p></div></button></SwiperSlide>)}</RailSwiper></section>;
}
function MissingHomeModules({ go }: { go:(to:string)=>void }) {
  const stories:[string,string,string,string][]=[['/assets/story-direct.webp','월간 과일','산지에서 식탁까지 복숭아 맛을 지키는 세 가지','온도 · 시간 · 배송'],['/assets/story-peach.webp','월간 과일','기대만큼 기준이 높은 프리미엄 복숭아','복숭아 큐레이션'],['/assets/story-watermelon.webp','월간 과일','수박의 기준을 뒤집는 블랙 프리미엄','SWT 흑수박'],['/assets/story-kiwi.webp','월간 과일','코끼리도 놀란 압도적 체급, 썬 골드키위 점보 국내 상륙','키위 큐레이션'],['/assets/story-blueberry.webp','월간 과일','대한민국이 인정한 31년 명인의 유기농 블루베리','블루베리로 장관상까지 휩쓴 비결은?']];
  ...
  return <>
    ...
    <ShowcaseRail className="peach-module" title="복숭아🍑 취향대로 골라요" subtitle="이름만큼 다양한 복숭아 라인업 준비완료!" items={sourceCatalog.peach.map(item=>({...item,review:'1000+'}))} go={go} swipe={false}/>
    <CakeDeal go={go}/>
    ...
    <HomeStories go={go} stories={stories}/>
    <section className="source-insta"><h2>인스타그램</h2><RailSwiper className="insta-rail">{instagramImages.map((image,i)=><SwiperSlide key={image}><a className="insta-item"><img src={image} alt={`인스타그램 게시물 ${i+1}`}/><div className="insta-overlay"><div className="insta-stats"><span className="insta-likes">❤️ {instaStats[i][0]}</span><span className="insta-comments">💬 {instaStats[i][1]}</span></div></div></a></SwiperSlide>)}</RailSwiper></section>
  </>;
}
function App() {
